...
""")
    print()
    print(
        "Note that any master-password entered will be taken even if it is just 1 character.If you want to be safe, follow the requirements")
    time.sleep(1)
    ...
    time.sleep(2)
    print(
        "Password saved!")  # The master-password has been saved in hash format and it cannot be decoded by any software unless its too predictable like '1234'
...
""")
        ...
        ch = input(
            "Do you want to look up info for another site?(y/n):")  # Provides an option to look up data of another account as well
...
""")
    ...
    if pass_choice.startswith('1'):
        ...
        test_pass = input(
            "Enter the password you want to use :")  # takes a password entered by user and verifies if satisfies all requirements to ensure safety
...
def inserting_values(table_name):
    ...
    insert_values = "INSERT INTO " + table_name + " (URL, Website, UserName, Password) " + "VALUES (" + "'" + url + "'" + ", " +\
...
def otp_verify():
    ...
    if otp_check == OTP1:
        ...
        print(
            "Quiting Program abruptly...")  # if user fails in verifying the OTP then the program shuts down and has to be restarted
        sys.exit()
...
def forgot_pass():
    ...
    otp_send()
    print(
        "An email has been sent to your registered mail with an OTP!")  # sends a mail to reset password by verifying OTP
    otp_verify()
...
""")
    ...
    print("""Contact Us: 
email: mypersonalpass21@gmail.com
github: https://github.com/Mat-Geo/LockXPro
""")
    print("Thank-you 😊")
