...
# Flow of execution
# PATH 1: new_or_login() -> new_user() -> intro() -> new_mast_pass() -> new_connection() -> table()
# -> password_req() -> passwd_encryption -> inserting_values() -> existing_user() -> verified() -> decrypt() ->
...
remembered by you at all times inorder to gain access to the manager. If the master-password is forgotten then you can 
choose forgot password option and an email will be sent to your registered email 
through which you can reset your master-password.
""")
    print()
...
iii) 1 digits
iiv) 1 special characters(min)
v) No space
""")
    regmail = input("Enter your email which will be used for verification :")
...
def table():
    ...
    cursor.execute("create table", table_name,
                   "(URL varchar(122),Website varchar(30),UserName varchar(14),Password varchar(201) PRIMARY KEY)")
    inserting_values()
...
i) At least 15 characters
ii) 1 uppercase letter(min)
iii) 2 digits(min)
iv) 2 special characters(min)
...
def inserting_values():
    ...
    sec_pwd = password_req()
    encrypted_pwd = str(passwd_encryption())
...
def passwd_encryption():
    global encryption
    global encrypted_pass
    key = Fernet.generate_key()
    encryption = Fernet(key)
    encrypted_pass = encryption.encrypt(sec_pwd.encode())
